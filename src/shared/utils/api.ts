...
class ApiClient {
  ...
  async createMarketplace(marketplace: MarketplaceFormData | { name: string; type: string; store_url?: string; settings?: any }): Promise<Marketplace> {
    const response = await this.request<Marketplace>('/api/marketplaces', {
      method: 'POST',
  ...
  async testMyntraConnection(marketplaceId: string): Promise<{ success: boolean; message: string; latency?: number }> {
    const response = await this.request<{ success: boolean; message: string; latency?: number }>(`/api/myntra/${marketplaceId}/test-connection`, {
      method: 'POST',
    });
    if (!response) throw new Error('Failed to test Myntra connection');
    return response;
  }
